...
/*
Every entry is resolved at startup and must succeed or the whole load fails, and nothing
calls glScissor, which is kept as core GL 1.0 for the editor's viewport clipping that no
build here reaches although every build would need it if it did.
*/
...
GL1_PROC(void, glPopMatrix, (void))
GL1_PROC(void, glPushMatrix, (void))
...
//Corrected from (HGLRC) to the real (HDC,int).
//A wrong prototype would unbalance the stack.
GL1_PROC(HGLRC, wglCreateLayerContext, (HDC, int))
