...
void CGClip::ClipTri(cl_tri_t &clTri) {
	...
	for (cpNum = 0, cpBit = 0x1; cpBitsRemain != 0; cpNum++, cpBit <<= 1) {
		...
		if (cpBit & cpBitsRemain) {
			...
			//Room for a convex polygon crossing each of the six planes once. Degenerate input
			//truncates; only the closest depth is read.
			const unsigned int MAX_PTS_OUT = 3 + NUM_CP;
...
unsigned int CGClip::GetHitNameStackSize(void) {
	return (unsigned int)m_hitNameStack.size();
}
...
void CGClip::SelectDrawLine(const vec3_t *pLnPts) {
	...
	for (u = 0; u < clLn.numPts; u++) {
		...
		//Ties go to the later object, as originally.
