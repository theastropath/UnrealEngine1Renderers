...
/*
Define UTGLR_RBTREE_REUSE_NODES to keep freed nodes for reuse; these trees churn from inside
draw calls. Off by default: OpenGL recycles, D3D9 goes to the heap.
*/
...
public:
#ifdef UTGLR_RBTREE_REUSE_NODES
	rbtree_allocator() {
		m_pFree = 0;
	}
	~rbtree_allocator() {
		while (m_pFree != 0) {
			node_t *pNode = m_pFree;
			m_pFree = pNode->pParent;
			delete pNode;
		}
	}
	...
	node_t *alloc_node(void) {
		...
		//Read before the reconstruction below writes pParent
		m_pFree = pNode->pParent;
...
private:
	rbtree_allocator(const rbtree_allocator &);
	rbtree_allocator &operator=(const rbtree_allocator &);
...
public:
	...
	//Empty on purpose: the owner must clear() with the allocator the nodes came from.
	~rbtree() {
	}
