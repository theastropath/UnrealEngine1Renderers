...
Shader_Line::Shader_Line(bool instanced) :
	Shader_Unreal(),
	instanced(instanced) {
...
bool Shader_Line::compile(const D3D10_SHADER_MACRO *macros, DWORD shaderFlags) {
	...
	if (instanced) {
		D3D10_INPUT_ELEMENT_DESC layoutDesc[] = {
			...
			{ "BLENDINDICES", 1, DXGI_FORMAT_R32_UINT, 1, D3D10_APPEND_ALIGNED_ELEMENT, D3D10_INPUT_PER_INSTANCE_DATA, 1 },
		};
		return Shader_Unreal::compileUnrealShader(L"d3d10drv\\line.fx", macros, shaderFlags, layoutDesc, sizeof(layoutDesc) / sizeof(layoutDesc[0])) != 0;
	}
	...
	if (!Shader_Unreal::compileUnrealShader(L"d3d10drv\\line.fx", macros, shaderFlags, layoutDesc, sizeof(layoutDesc) / sizeof(layoutDesc[0])))
		return false;
